...
function hash() {
    ...
    const messageArr = new Uint8Array(
        dataview.buffer,
        contextArr.byteOffset + contextArr.byteLength,
        message.length,
    );
    Buffer.from(message).copy(messageArr); // Copying message to shared buffer
    const hash = new Uint8Array(
        dataview.buffer,
        messageArr.byteOffset + // Creating a typed array as hydro_hash_hash expects i32s
    ...
    hydro_hash_hash(
        hash.byteOffset,
        hash.length,
        messageArr.byteOffset, // Call the imported hashing function
        messageArr.byteLength,
...
function keyed_hash() {
    ...
    const key = new Uint8Array(
        dataview.buffer,
        messageArr.byteOffset + // Creating buffer for sharing key
            messageArr.byteLength,
        32,
    );
    ...
    hydro_hash_hash(
        ...
        keyedhash.length,
        messageArr.byteOffset, // Hashing message with key
        messageArr.byteLength,
        contextArr.byteOffset,
        key.byteOffset,
    );
    ...
    hydro_hash_hash(
        keyedhash.byteOffset,
        keyedhash.length,
        messageArr.byteOffset, // Hashing message with key again
        messageArr.byteLength,
        contextArr.byteOffset,
        key.byteOffset,
    );
    ...
    if (khash1 == khash2)
        // Checking that the same hash is generated
    ...
    hydro_hash_hash(
        keyedhash.byteOffset,
        keyedhash.length,
        messageArr.byteOffset, // Hashing message with presetKey
        messageArr.byteLength,
        contextArr.byteOffset,
        key.byteOffset,
    );
...
function public_key_signing() {
    ...
    const keypair = new Uint8Array(
        dataview.buffer,
        messageArr.byteOffset + // Reserving memory for the keypair
            messageArr.byteLength,
    ...
    const signature = new Uint8Array(
        dataview.buffer,
        keypair.byteOffset + keypair.byteLength, // Reserving memory for the signature
        hydro_sign_BYTES,
    );

    hydro_sign_create(
        signature.byteOffset,
        messageArr.byteOffset, // Creating signature of message with secret key
        messageArr.byteLength,
        contextArr.byteOffset,
    ...
    const res = hydro_sign_verify(
        signature.byteOffset,
        messageArr.byteOffset, // Verifying signature with public key
        messageArr.byteLength,
        contextArr.byteOffset,
        keypair.byteOffset,
    );
...
function symmetric_encryption() {
    ...
    const {
        ...
        hydro_secretbox_encrypt, // Importing libhydrogen's secretbox keygen and encrypt and decrypt functions
        hydro_secretbox_decrypt,
    } = instance.exports;

    const key = new Uint8Array(
        dataview.buffer,
        messageArr.byteOffset + messageArr.byteLength, // Reserving buffer for the key
        keyLength,
    );
    ...
    const ciphertext = new Uint8Array(
        dataview.buffer,
        key.byteOffset + key.byteLength, // Reserving buffer for the cipher text
        cipherTextLength,
    );

    hydro_secretbox_encrypt(
        ciphertext.byteOffset,
        messageArr.byteOffset, // Enciphering single message (thus use of msg_id 0n -- 'n' as libhydrogen expects i64)
        messageArr.byteLength,
        0n,
        contextArr.byteOffset,
        key.byteOffset,
    ...
    const decryptedPlaintext = new Uint8Array(
        dataview.buffer,
        ciphertext.byteOffset + // Reserving buffer for the decrypted plain text
            ciphertext.byteLength,
        ciphertext.byteLength - hydro_secretbox_HEADERBYTES,
    );

    const res = hydro_secretbox_decrypt(
        decryptedPlaintext.byteOffset,
        ciphertext.byteOffset, // Deciphering single message (thus use of msg_id 0n -- 'n' as libhydrogen expects i64)
        ciphertext.byteLength,
        0n,
        contextArr.byteOffset,
        key.byteOffset,
    ...
    if (res == 0) {
        // As secretbox is an authenticated encryption (AEAD) algorithm we check that the ciphertext was authentic
...
function symmetric_encryption_via_asymmetric_key_exchange() {
  ...
  */
    ...
    const hash_state_BYTES =
        hydro_kx_PUBLICKEYBYTES + hydro_kx_SECRETKEYBYTES + (32 / 8) * 12 + 1 + 3;
    ...
    const alice = {
        ...
        session: {
            rx: new Uint8Array(
                dataview.buffer,
                hydro_kx_PUBLICKEYBYTES + hydro_kx_SECRETKEYBYTES + hash_state_BYTES,
    ...
    const bob = {
        static: {
            pk: new Uint8Array(
                dataview.buffer,
                alice.session.tx.byteOffset + alice.session.tx.byteLength,
                hydro_kx_PUBLICKEYBYTES,
            ),
            sk: new Uint8Array(
                dataview.buffer,
                alice.session.tx.byteOffset + alice.session.tx.byteLength + hydro_kx_PUBLICKEYBYTES,
                hydro_kx_SECRETKEYBYTES,
            ),
        },
        session: {
    ...
    const packets = {
        1: new Uint8Array(
            dataview.buffer,
            bob.session.tx.byteOffset + bob.session.tx.byteLength,
            hydro_kx_KK_PACKET1BYTES,
        ),
        2: new Uint8Array(
            dataview.buffer,
            bob.session.tx.byteOffset + bob.session.tx.byteLength + hydro_kx_KK_PACKET1BYTES,
    ...
    hydro_kx_kk_1(
        ...
        bob.static.pk.byteOffset, // Performing kk_1 (Generating alice's ephemeral keypair and packet 1)
    ...
    hydro_kx_kk_2(
        ...
        packets[1].byteOffset, // Performing kk_2 (Generating bob's response packet 2 and his copy of session keys)
        alice.static.pk.byteOffset,
    ...
    hydro_kx_kk_3(
        ...
        packets[2].byteOffset, // Performing kk_3 (Generating Alice's copy of session keys)
        alice.static.pk.byteOffset,
    );
    console.log('---------------ALICE---------------');
    console.dir(alice, { maxArrayLength: null });
    ...
    const context = 'Examples\0'; // libhydrogen's namespacing concept needs to be null terminated as context arg expected to char[]
    const contextArr = new Uint8Array(
        dataview.buffer,
        packets[2].byteOffset + packets[2].byteLength,
        context.length,
    );
    Buffer.from(context).copy(contextArr);

    const message1 = 'Hello Bob';
    const messageArr1 = new Uint8Array(
        dataview.buffer,
        contextArr.byteOffset + contextArr.byteLength,
        message1.length,
    );
    ...
    const ciphertext1 = new Uint8Array(
        dataview.buffer,
        messageArr1.byteOffset + // Reserving buffer for the cipher text
            messageArr1.byteLength,
        hydro_secretbox_HEADERBYTES + Buffer.from(message1).length,
    );

    hydro_secretbox_encrypt(
        ciphertext1.byteOffset,
        messageArr1.byteOffset, // Enciphering single message (thus use of msg_id 0n -- 'n' as libhydrogen expects i64)
        messageArr1.byteLength,
        0n,
        contextArr.byteOffset,
        alice.session.tx.byteOffset,
    ...
    const decryptedPlaintext1 = new Uint8Array(
        dataview.buffer,
        ciphertext1.byteOffset + // Reserving buffer for the decrypted plain text
            ciphertext1.byteLength,
        ciphertext1.byteLength - hydro_secretbox_HEADERBYTES,
    );

    const res1 = hydro_secretbox_decrypt(
        decryptedPlaintext1.byteOffset,
        ciphertext1.byteOffset, // Deciphering single message (thus use of msg_id 0n -- 'n' as libhydrogen expects i64)
        ciphertext1.byteLength,
        0n,
        contextArr.byteOffset,
        bob.session.rx.byteOffset,
    ...
    const messageArr2 = new Uint8Array(
        dataview.buffer,
        decryptedPlaintext1.byteOffset + decryptedPlaintext1.byteLength,
        message2.length,
    );
    ...
    const ciphertext2 = new Uint8Array(
        dataview.buffer,
        messageArr2.byteOffset + // Reserving buffer for the cipher text
            messageArr2.byteLength,
        hydro_secretbox_HEADERBYTES + Buffer.from(message2).length,
    );

    hydro_secretbox_encrypt(
        ciphertext2.byteOffset,
        messageArr2.byteOffset, // Enciphering single message (thus use of msg_id 0n -- 'n' as libhydrogen expects i64)
        messageArr2.byteLength,
        0n,
        contextArr.byteOffset,
    ...
    const decryptedPlaintext2 = new Uint8Array(
        dataview.buffer,
        ciphertext2.byteOffset + // Reserving buffer for the decrypted plain text
            ciphertext2.byteLength,
        ciphertext2.byteLength - hydro_secretbox_HEADERBYTES,
    );

    const res2 = hydro_secretbox_decrypt(
        decryptedPlaintext2.byteOffset,
        ciphertext2.byteOffset, // Deciphering single message (thus use of msg_id 0n -- 'n' as libhydrogen expects i64)
        ciphertext2.byteLength,
        0n,
        contextArr.byteOffset,
        alice.session.rx.byteOffset,
